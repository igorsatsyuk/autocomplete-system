...
- Use shared runtime identifiers from `backend/common/src/main/java/lt/satsyuk/common/` via `KafkaTopics` and `RedisKeys`.
...
- Treat backend modules as one Maven reactor under `backend/`; use `mvn -f backend/pom.xml -pl <module> -am ...` for targeted builds.
...
- Add new Kafka or Redis identifiers in `backend/common` first, then reference them via configuration defaults.
- Preserve Debezium message handling through `payload.after`; do not assume a flat CDC JSON payload.
- When changing schema, update both `backend/search-service/src/main/resources/db/migration/` and `infra/postgres/` so Compose bootstrap matches Flyway.
...
- Don't hardcode `search-events`, `db-changes.public.search_stats`, or `autocomplete:` outside `backend/common` defaults.
...
- Reuse identifiers from `backend/common` before adding any topic names, regexes, or Redis prefixes.
...
- Key files: `backend/search-service/.../SearchController.java`, `backend/search-service/.../SearchEventProducer.java`, `backend/search-service/.../SearchStatsTopology.java`, `backend/cdc-service/.../DebeziumConsumer.java`, `backend/cdc-service/.../RedisSearchUpdater.java`, `backend/autocomplete-service/.../AutocompleteQueryService.java`, `frontend/src/app/services/autocomplete.service.ts`.
